    ...
    <Box mb={40} pos={'relative'} c={'white'} p={20}>
      ...
      <Box pos={'relative'} style={{ zIndex: 10 }} mx={'auto'}>
        <Flex justify={'space-between'} align={'center'} direction={'column'} w={'100%'} mx={'auto'}>
          ...
          <Text w={'80%'} ta={'center'} my={20} style={{ maxWidth: '700px' }} fz={{ base: 12, xs: 18 }}>
          ...
          <Flex direction={{ base: 'column', xs: 'row' }} gap={20} my={25}>
            <Box component="button" fw={'bold'} fz={{ base: 10, xs: 14, sm: 16 }} className={`${classes.btnPrimary}`}>
              {' '}
              BECOME A MEMBER
            </Box>
            <Box component="button" fw={'bold'} fz={{ base: 10, xs: 14, sm: 16 }} className={`${classes.btnSecondary}`}>
              {' '}
              HOW TO EARN POINTS
            </Box>
          </Flex>
